...
export async function analyzeWithLLM(
  ...
  const parsed = tryParseJSON<AnalysisResult>(raw);
  if (!parsed) throw new Error(`PII analyzer returned unparseable JSON: ${raw.slice(0, 200)}`);
